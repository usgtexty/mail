<template>
	<form id="account-form" @submit.prevent="onSubmit">
		<Tabs
			:options="{useUrlFragment: false, defaultTabHash: settingsPage && oauthProviders.length <= 0 ? 'manual' : 'oauth'}"
			cache-lifetime="0"
			@changed="onModeChanged">
			<Tab id="oauth" key="oauth" :name="t('mail', 'OAuth')">
				<div v-if="oauthProviders.microsoft" class="auth-provider" :title="t('mail', 'Sign in with Microsoft')">
					<a :href="oauthProviders.microsoft">
...
export default {
	...
	computed: {
		...
		googleLoginImage() {
			return generateFilePath('mail', 'img', 'sign-in-with-google.png')
		}
	},
	methods: {
		onModeChanged(e) {
