<template>
	<form id="account-form" @submit.prevent="onSubmit">
		<Tabs
			:options="{useUrlFragment: false, defaultTabHash: settingsPage ? 'manual' : 'oauth'}"
			cache-lifetime="0"
			@changed="onModeChanged">
			<Tab id="oauth" key="oauth" :name="t('mail', 'OAuth')">
				<div class="auth-provider" :title="t('mail', 'Sign in with Microsoft')">
				...
				<div class="auth-provider" :title="t('mail', 'Sign in with Google')">
